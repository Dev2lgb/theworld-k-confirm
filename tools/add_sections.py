...
def add_sections(body):
    ...
    block = (
        ...
        + p('<b>이 자료만은 저희가 대신 만들어 드릴 수 없습니다.</b> 접수증이나 자격증 서식은 '
            '저희가 초안을 만들어 보여 드릴 수 있지만, 두 자격이 어떤 자격인지는 '
            '더월드에서만 아시는 내용입니다. 내용을 주셔야 화면을 만들 수 있습니다.')
        + think([
            '종목마다 아래 내용을 알려 주시면, 문장을 다듬고 화면을 만드는 일은 저희가 하겠습니다. '
            '완성된 글이 아니어도 됩니다. 항목마다 한두 줄씩만 적어 주셔도 됩니다.',
            '글로 적기 번거로우시면 전화로 말씀해 주십시오. 저희가 받아 적어 정리한 뒤 '
            '보여 드리고, 틀린 곳만 고쳐 주시면 됩니다.',
        ...
        + options([
            '종목마다 위 내용을 적어서 드리겠습니다',
            '이미 있는 자료를 드리겠습니다 (기존 홈페이지 글, 브로슈어, 공문 등)',
            '전화로 말씀드릴 테니 받아 적어 정리해 주십시오',
            '기타',
        ])
        + writein()
    )
